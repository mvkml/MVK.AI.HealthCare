...
public class OrganizationBL : IOrganizationBL
{
    ...
    private async Task<ImportResult> RunImport(Stream csvStream, Func<List<OrganizationItem>, ImportResult, Task> flush)
    {
    ...
    private async Task FlushUpsertBatch(List<OrganizationItem> batch, ImportResult result)
    {
        try
        {
            await _organizationRepository.UpsertBatch(batch);
            result.InsertedCount += batch.Count;
        }
        catch (Exception ex)
        {
