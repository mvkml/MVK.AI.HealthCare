...
public class ProcedureBL : IProcedureBL
{
    ...
    private async Task<ImportResult> RunImport(Stream csvStream, Func<List<ProcedureItem>, ImportResult, Task> flush)
    {
    ...
    private async Task FlushUpsertBatch(List<ProcedureItem> batch, ImportResult result)
    {
        try
        {
            await _procedureRepository.UpsertBatch(batch);
            result.InsertedCount += batch.Count;
        }
        catch (Exception ex)
        {
