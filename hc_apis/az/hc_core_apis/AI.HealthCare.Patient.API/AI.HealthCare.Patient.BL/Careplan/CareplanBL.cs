...
public class CareplanBL : ICareplanBL
{
    ...
    private async Task<ImportResult> RunImport(Stream csvStream, Func<List<CareplanItem>, ImportResult, Task> flush)
    {
    ...
    private async Task FlushUpsertBatch(List<CareplanItem> batch, ImportResult result)
    {
        try
        {
            await _careplanRepository.UpsertBatch(batch);
            result.InsertedCount += batch.Count;
        }
        catch (Exception ex)
        {
