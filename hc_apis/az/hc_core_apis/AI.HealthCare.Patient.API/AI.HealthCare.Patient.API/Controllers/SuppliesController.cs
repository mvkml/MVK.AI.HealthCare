...
public class SuppliesController : ControllerBase
{
    ...
    /// <summary>Bulk upserts supplies from a CSV file (Synthea supplies.csv format). Rows matching an existing Patient+Encounter+Code combination are updated in place; others are inserted. Matching Patient and Encounter records must already exist.</summary>
    ...
    public async Task<ActionResult<ImportResult>> ImportUpsert(IFormFile file)
    {
        ...
        var result = await _supplyBL.ImportUpsert(file.OpenReadStream());
        return Ok(result);
    }
