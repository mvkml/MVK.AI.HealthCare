...
public class MedicationsController : ControllerBase
{
    ...
    /// <summary>Bulk upserts medications from a CSV file (Synthea medications.csv format). Rows matching an existing Patient+Encounter+Code combination are updated in place; others are inserted. Matching Patient, Payer, and Encounter records must already exist.</summary>
    ...
    public async Task<ActionResult<ImportResult>> ImportUpsert(IFormFile file)
    {
        ...
        var result = await _medicationBL.ImportUpsert(file.OpenReadStream());
        return Ok(result);
    }
