...
public class ProvidersController : ControllerBase
{
    ...
    /// <summary>Bulk upserts providers from a CSV file (Synthea providers.csv format). Rows whose Id already exists are updated in place; new Ids are inserted. Matching Organization records must already exist.</summary>
    ...
    public async Task<ActionResult<ImportResult>> ImportUpsert(IFormFile file)
    {
        ...
        var result = await _providerBL.ImportUpsert(file.OpenReadStream());
        return Ok(result);
    }
