...
test.describe('Signup (Patient)', () => {
  test('valid signup shows a success message', async ({ page }) => {
    const user = buildTestUser({ persona: 'patient' });

    await page.goto('/signup');
    await page.locator('input[name="fullName"]').fill(user.fullName);
    await page.locator('input[name="email"]').fill(user.email);
    await page.locator('select[name="persona"]').selectOption('patient');
  ...
  test('password under 8 characters shows a client-side error', async ({ page }) => {
    const user = buildTestUser({ persona: 'patient', password: 'short1' });
  ...
  test('an invalid email format is rejected by the backend (no client-side email check)', async ({ page }) => {
    const user = buildTestUser({ persona: 'patient', email: 'not-an-email' });
  ...
  test('signing up as Patient lands that account on the Patient home view after login', async ({ page }) => {
    const user = buildTestUser({ persona: 'patient' });
    ...
    await expect(page.getByRole('heading', { name: 'My Health Assistant' })).toBeVisible();
  });
